...
import { useGetCallerUserProfile, useUpgradeSubscriptionTier } from '../hooks/useQueries';
...
export default function PlansPage() {
  ...
  const isUpgrading = (tier: SubscriptionTier) => upgradeMutation.isPending && upgradeMutation.variables === tier;
    ...
    <div className="min-h-screen">
      ...
      <section className="relative bg-gradient-to-br from-primary/5 via-background to-accent/5 py-20 px-4 overflow-hidden">
        ...
        <div className="container mx-auto max-w-4xl relative text-center">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            Start for free and upgrade as you grow. All plans include unlimited buying and selling.
          </p>
        </div>
      </section>
      ...
      <section className="container mx-auto max-w-6xl px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {/* Starter */}
          <div className={`relative flex flex-col rounded-2xl border-2 bg-card shadow-soft transition-all duration-300 hover:shadow-medium overflow-hidden ${isCurrentTier(SubscriptionTier.starter) ? 'border-plan-starter ring-2 ring-plan-starter/30' : 'border-border'}`}>
            ...
            <div className="p-8 pb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-foreground font-display">Starter</h2>
                  <span className="text-xs font-medium text-plan-starter uppercase tracking-wider">Free Forever</span>
                </div>
              </div>
              <div className="mb-6">
          ...
          {/* Pro */}
          <div className={`relative flex flex-col rounded-2xl border-2 bg-card shadow-soft transition-all duration-300 hover:shadow-medium overflow-hidden ${isCurrentTier(SubscriptionTier.pro) ? 'border-plan-pro ring-2 ring-plan-pro/30' : 'border-plan-pro/40'}`}>
            ...
            <div className="p-8 pb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-foreground font-display">Pro</h2>
                  <span className="text-xs font-medium text-plan-pro uppercase tracking-wider">Most Popular</span>
                </div>
              </div>
              <div className="mb-6">
              ...
              <ul className="space-y-3 mb-8">
                {proFeatures.map((feature) => (
                  <li key={feature} className={`flex items-start gap-2.5 text-sm ${feature.startsWith('Everything') ? 'text-foreground font-medium' : 'text-muted-foreground'}`}>
            ...
            <div className="mt-auto px-8 pb-8">
              {isCurrentTier(SubscriptionTier.pro) ? (
                ...
                <Button
                  ...
                  disabled={upgradeMutation.isPending || profileLoading || !isAuthenticated}
                >
          ...
          {/* Max */}
          <div className={`relative flex flex-col rounded-2xl border-2 bg-card shadow-soft transition-all duration-300 hover:shadow-medium overflow-hidden ${isCurrentTier(SubscriptionTier.max) ? 'border-plan-max ring-2 ring-plan-max/30' : 'border-plan-max/40'}`}>
            ...
            <div className="p-8 pb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-foreground font-display">Max</h2>
                  <span className="text-xs font-medium text-plan-max uppercase tracking-wider">Ultimate Power</span>
                </div>
              </div>
              <div className="mb-6">
        ...
        {!isAuthenticated && (
          <p className="text-center text-muted-foreground mt-8 text-sm">
            Please <button onClick={() => navigate({ to: '/' })} className="text-primary underline underline-offset-2 hover:no-underline">log in</button> to upgrade your plan.
          </p>
        )}
      </section>
    </div>
