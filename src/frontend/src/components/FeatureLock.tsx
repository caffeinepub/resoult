...
export default function FeatureLock({
  ...
  const tierBg =
    requiredTier === "pro"
      ? "bg-plan-pro/10 border-plan-pro/30"
      : "bg-plan-max/10 border-plan-max/30";
  const tierBadgeBg =
    requiredTier === "pro"
      ? "bg-plan-pro text-white"
      : "bg-plan-max text-white";

  return (
    <button
      type="button"
      className={`relative w-full text-left rounded-xl border-2 border-dashed ${tierBg} p-4 cursor-pointer group transition-all duration-200 hover:shadow-soft`}
      onClick={() => navigate({ to: "/plans" })}
      aria-label={`Upgrade to ${tierLabel} to unlock this feature`}
    >
      <div className="flex items-center gap-3">
        <div
          className={`p-2 rounded-lg ${requiredTier === "pro" ? "bg-plan-pro/15" : "bg-plan-max/15"}`}
        >
        ...
        <span
          className={`shrink-0 text-xs font-bold px-2.5 py-1 rounded-full ${tierBadgeBg}`}
        >
          {tierLabel}
        </span>
      </div>
